...
import { MagnifyingGlass, ThumbsUp, WarningCircle } from 'phosphor-react-native';
...
/**
 * 프로필 > 내가 좋아요한 작품 - 웹 my-likes-page 문법 이식.
 * 구조·상태 처리는 bookmarks와 동일 (제목 검색 + 2열 WorkCard + 상태 3종).
 */
export default function MyLikesScreen() {
  const [searchQuery, setSearchQuery] = useState('');
  const { data, isLoading, isError, refetch } = useMyLikes(0, 1000);
      ...
      <FlatList
        ...
        ListEmptyComponent={
          isLoading ? (
            ...
            <EmptyState
              icon={<ThumbsUp size={44} color={Palette.ink3} />}
              title="아직 좋아요한 작품이 없어요"
              description="좋았던 작품에 마음을 남겨보세요."
              action={
                <EmptyStateAction
                  label="작품 둘러보기"
