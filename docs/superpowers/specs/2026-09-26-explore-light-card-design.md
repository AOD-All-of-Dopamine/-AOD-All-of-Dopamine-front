...
상태: **v2 · 구현됨** — v1 코드 대조 독립 검수 반영(사실 11 · 심각 2 · 보통 8 · 사소 9 · 빠진 것 9), 구현 후 독립 검수 반영(맨 아래 "검수 기록" · "구현 기록")
...
**브랜치**: 백엔드 `feature/explore-vote-count`(`b162fe7`) · 프론트 `feature/explore-light-card`. **백엔드를 먼저 배포한다**(없으면 모든 별점 · %가 숨는다).

### 설계와 달라진 점

| # | 설계 | 구현 | 이유 |
|---|---|---|---|
| I1 | 게임 열: 목록형 → 3 → 4 → 5 (경계 "대략") | **목록형(767px 이하) → 3(768px~) → 4(본문 760px~) → 5(본문 1000px~)** | 4열을 본문 700px 에 두면 748~767px 목록형 구간에서도 켜지고, 768 · 1024px 에서 카드가 166px 로 좁아 신호 줄이 잘렸다(구현 검수). 5열은 가장 긴 판정 줄(약 186px)이 들어가는 1000px |
| I2 | 세로 열: 2 → 3 → 4 → 5 → 6 | 본문 440 · 600 · 760 · 920px 경계 | 실측 카드 폭 149~179px(360~1440px) |
| I3 | 상세 문구: 중복 정리 | 판정이 **없으면**(판정 전 · 문구 없음) "평가 N개" 한 번만, %와 평가 수 숨김. 모바일 상세도 같게 | 문구 없음(null)일 때 "긍정적" 고정 + 적은 표본 % 가 남아 있었다(구현 검수) |
| I4 | `workLiteSignal` 원천 | shared `constants/workSignal.ts` 에 `ageLabel` · `orderedWatchLabels` · `signalText` 도 | 웹 `workCardInfo` 와 규칙 원천 하나 |
| I5 | — | 컬렉션 매핑 두 곳(`collectionItemInfo.ts` · `ShelfPlayground.tsx`)에 새 필드 | 빠지면 컬렉션 별점 · %가 모두 숨는다 |

### 구현 후 독립 검수 (심각 0 · 보통 2 · 사소 4)

| # | 문제 | 반영 |
|---|---|---|
| M1 | 게임 그리드: 뷰포트 · 컨테이너 쿼리가 겹쳐 748~767px 에 목록형 카드 4열 | I1 |
| M2 | 게임 3열이 쓰이지 않고 768 · 1024px 카드 166px, 긴 판정 줄 잘림 | I1 |
| 사소 | 모바일 상세 중복 문구 · 웹 상세 문구 없음(null) 처리 · 주석(256px 레일, 한 쪽 30 설명) | 반영 |
| 남김 | **모바일 카드(`workCardInfo` · `GameCompactCard` · 컬렉션)에는 적은 표본 숨김이 없다** — 설계 범위(web)는 지켰지만 웹 · 앱 사이 표시가 다르다. 모바일 컬렉션 매핑도 새 필드를 넘기지 않는다 | **후속** |

### 확인한 것

- 백엔드: api 테스트 **319개** 통과 — 로컬 PostgreSQL 로 REST Docs 테스트(Interaction · Collection)까지 실제로 돌렸다(CI 는 `-x test` 라 돌리지 않는다). 새 `WorkSummaryEnrichTest` 6건.
- 프론트: shared 테스트 **248개**(신규 `workSignal.test.ts` — 한글화 · 표본 기준 · 연령 · OTT 순서 · 분야별 신호 · 중점 규칙), lint · typecheck(웹 · 모바일) · 빌드. 바꾼 줄에 새 게이트 위반 없음.
- **브라우저 41/41**(헤드리스 크롬, 운영 API 를 Playwright 로 대신 불러 새 필드는 상세 API 로 채움 — 백엔드 배포 뒤 모습):
  폭 360 · 375 · 768 · 1024 · 1200 · 1280 · 1440 × 게임 · 영화 — 한 쪽 30장 · 카드 최소 폭 · 신호 줄 잘림 0, 레일 216px · 제목 20px · "평가 수 (Steam)" · 영어 "user review" 없음 · 레일 장르 6개 · 페이지 오류 0.
  첫 화면 카드: 게임 1440px 5 → **20**, 게임 375px 2 → **9**(목록형), 영화 1440px 5 → **12**, 영화 375px 2 → 4.
  최신 영화 30개 중 별점이 보이는 것은 2개(나머지는 투표 20개 미만) — 의도대로.
- **못 한 것**: 운영 데이터의 투표 수 분포 측정(열린 결정 2) · 스크린리더 실기기 · 모바일 앱 실기기.
