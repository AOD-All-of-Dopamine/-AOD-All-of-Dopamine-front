...
- [x] **6.1** `/new`: 날짜 그룹 타임라인(sticky h3 라벨, ReleaseRow 신설) + 출시 예정 월 그룹+DdayPill + 도메인 칩 URL 동기화. 알림 받기는 알림 API 부재로 생략(편차)
- [x] **6.2** 랭킹 수집 축 백엔드 실코드 확인 완료 — 기간(스냅샷 단일 upsert)·집계 기준(플랫폼당 1개)·장르(필드 없음)·델타(비교 원본 소실) 전부 미수집 → UI 미렌더 + 편차 기록. 날짜 헬퍼 `utils/releaseDate.ts` 공용화
- [x] **6.3** `/ranking`: PodiumCard 포디움(2·3위 단차) + RankRow 패널 + 도메인·플랫폼 칩(수집 원천 활성, 미수집 SoonBadge) + 실수집 기준 출처 캡션 5종(TMDB는 국내 OTT 제공작 정밀 문구)
- [x] **6.4** 게이트: 스펙 리뷰 ✅(편차 12건 백엔드 실코드 확정) + 품질 리뷰 ✅(I-1+M-7건 반영) + grep 0건 + 빌드 그린 + 실브라우저(타임라인·포디움 단차·준비 중 칩) — `docs/screenshots/phase6/`
