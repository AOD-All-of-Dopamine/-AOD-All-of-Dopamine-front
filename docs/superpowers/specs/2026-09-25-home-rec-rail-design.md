...
- **👍/👎**: **항상 보인다**(§2-4). 버튼 36px 이상, 둘 사이 간격 8px 이상(오조작 방지). 👍 는 `aria-pressed` 에 라벨 `"{제목} 좋아요"` **고정**
  (§2-4 의 "좋아요 취소" 전환은 쓰지 않는다 — 열린 결정 0), 👎 는 `"{제목} 관심 없어요"`. 아이콘 `ThumbsUp`·`ThumbsDown`(phosphor —
...
> **2026-09-26 변경 — 플래그 제거.** 빌드 플래그 없이 항상 켠다(`HOME_REC_ENABLED`·`VITE_HOME_REC` 삭제, 리뷰 섹션·모바일 세그먼트 코드 삭제).
> 추천 서버가 없거나 허용 목록 밖이면 릴은 대체(인기) 모드로 보인다. 아래는 플래그를 두던 때의 기록이다.
...
0. ~~**👍 의 `aria-pressed` + 바뀌는 라벨**~~ — **결정(2026-09-25): 라벨 "좋아요" 고정 + `aria-pressed` 만.** "좋아요"/"좋아요 취소"에 `aria-pressed` 를 함께 쓰면 "좋아요 취소, 눌림"처럼 읽힌다. 저장소의 다른 토글(`OnboardingWorkTile`)과 같은 관례로 맞췄다. REC_TAB_DESIGN §2-4 의 라벨 전환 서술과 추천 탭 카드(`RecCardTile`, 아직 하트)는 U4 에서 같이 고친다.
...
| N5 | 사소 | 👍 의 `aria-pressed` + 바뀌는 라벨이 저장소 관례와 다르다 | **열린 결정 0** 으로 올렸고, 라벨 고정으로 결정·반영했다 |
