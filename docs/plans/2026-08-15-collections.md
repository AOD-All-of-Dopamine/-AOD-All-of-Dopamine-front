...
### 게이트 (태스크별 커밋 전)
- [x] BE: 컴파일 + 테스트 그린(53건) / FE: npm run build + grep 게이트 - 각 태스크·리뷰 사이클마다 재확인
- [x] FE: 데스크톱+모바일(390px) 실화면 목업 대조 - 발견/상세/생성/담기 팝오버·모바일 발견·6탭 (오케스트레이터)
- [x] E2E: 생성(게임·테라코타·공개) → 담기(토스트+서버 아이템·Steam 보강 확인) → 좋아요 0→1 → 발견 노출(♥1·조회 3) - 로컬 실서버 (오케스트레이터)
...
- C-FE2: 편집은 드래프트 방식 - 아이템 제거도 저장 시점 일괄(DELETE, 404는 멱등 통과)로 미뤄 취소가 전부 복원되게 했다 (플랜의 저장 일괄 목록에 제거를 추가). 저장 순서: 제거 DELETE → PUT order(상대 순서 바뀐 경우만) → 코멘트 PATCH(바뀐 것만) → 메타 PATCH(바뀐 필드만). 실패 시 드래프트 유지 + 에러 배너 + 재시도 - 단 전체 재실행 안전은 5xx/네트워크 한정이고, 4xx(타 탭 아이템 추가로 PUT order 집합 불일치 400 등)는 재시도가 무의미해 "최신 상태로 새로고침"(재조회 + 편집기 재마운트로 드래프트 재구성) 분기. 부분 성공으로 캐시가 어긋나지 않게 저장 실패 시 상세를 무효화한다(활성 재조회 조회수 +1은 정합성 비용으로 수용).
...
- C-FE2: 드래프트 편집 중 브라우저 뒤로가기/새로고침 가드 부재 (X·취소 버튼만 ConfirmDialog로 확인 - beforeunload/라우터 블로커는 백로그).
- C-FE2: (인지) 담기 시트 내 토스트는 top-layer dialog 자식의 fixed 포지셔닝에 의존 - dialog 자체에 transform 전환이 있어 containing block이 dialog가 되므로, 시트 패널 기준으로 위치한다(현 시각은 수용 범위). dialog 스타일 변경 시 위치가 틀어질 수 있음.
