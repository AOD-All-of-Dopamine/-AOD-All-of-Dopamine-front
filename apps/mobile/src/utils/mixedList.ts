/**
 * FlatList 행 시퀀스 파생 - 웹 utils/mixedGrid.ts의 RN 대응. 웹은 CSS 그리드
 * 스팬(게임 페어 셀)으로 풀지만, RN FlatList는 한 목록에서 numColumns를
 * 아이템별로 섞을 수 없어 목록 자체를 "행" 단위로 재구성한다:
 * - mixed 모드(검색 - 탐색 전체 탭 제거 후 유일한 혼합 표면)의 게임 = 풀폭
 *   컴팩트 가로 행 1개 (목업 1-C의 <lg·앱 규칙)
 ...
 * (탐색 도메인 탭 - 기존 numColumns=2 그리드와 같은 시각).
 */
