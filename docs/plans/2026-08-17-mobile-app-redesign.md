...
- [x] expo web 실화면 목업 대조 (오케스트레이터) - 홈(히어로·릴·리뷰 행·6탭)/탐색(칩·2열
      카드·필터 시트 즉시 적용 894→359 라이브 카운트·활성 칩)/컬렉션 탭 레이아웃 실데이터
      검증 통과. 참고: 웹 타깃 한정으로 ExpoSecureStore 미지원이라 인증 필요 호출이 실패
      (네이티브 전용 저장소 - 리디자인 전부터의 플랫폼 한계, 실기기 무관). CI 모드로 실행
      (Windows Metro EMFILE 회피).
- [ ] 실기기/에뮬레이터 확인은 사용자 몫 (Expo Go) - 로그인·컬렉션 상호작용 포함 전 플로우
